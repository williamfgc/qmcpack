...
{
/** class to match std::complex<T> spline with BsplineSet::ValueType (complex)
 * SPOs
 * @tparam T precision of spline
 *
 * Requires temporage storage and multiplication of phase vectors
 * The internal storage of complex spline coefficients uses double sized real
 * arrays of T type, aligned and padded. All the output orbitals are complex.
 */
template <class T>
...
public:
    ...
    {
        return std::make_unique<SplineC2CT<T>>(*this);
    }
    ...
    */
    ...
    gather_tables(Communicate* comm)
    {
        ...
        gatherv(comm, SplineInst->getSplinePtr(),
            SplineInst->getSplinePtr()->z_stride, offset);
    }
    ...
    void
    evaluateValue(
        const ParticleSet& P, const int iat, ValueVector& psi) override;
    ...
    void
    evaluateVGL(const ParticleSet& P, const int iat, ValueVector& psi,
    ...
    void
    evaluateVGH(const ParticleSet& P, const int iat, ValueVector& psi,
    ...
    void
    evaluateVGHGH(const ParticleSet& P, const int iat, ValueVector& psi,
