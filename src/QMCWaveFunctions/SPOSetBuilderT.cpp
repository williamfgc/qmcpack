...
//                    Illinois at Urbana-Champaign Raymond Clay III,
...
{
template <typename T>
SPOSetBuilderT<T>::SPOSetBuilderT(
    const std::string& type_name, Communicate* comm) :
    MPIObjectBase(comm),
    legacy(true),
...
template <typename T>
void
SPOSetBuilderT<T>::reserve_states(int nsets)
{
...
SPOSetBuilderT<T>::createSPOSet(xmlNodePtr cur, SPOSetInputInfo& input_info)
{
    myComm->barrier_and_abort(
        "BasisSetBase::createSPOSet(cur,input_info) has not been implemented");
    return 0;
}

template <typename T>
std::unique_ptr<SPOSetT<T>>
SPOSetBuilderT<T>::createSPOSet(xmlNodePtr cur)
{
    ...
    app_summary() << "     ------------------------------" << std::endl;
    app_summary() << "      Name: " << spo_object_name
                  << "   Type: " << type_name_
    ...
    if (spo_object_name.empty())
        myComm->barrier_and_abort(
            "SPOSet object \"name\" attribute not given in the input!");
    ...
    if (!sposet)
        myComm->barrier_and_abort(
            "SPOSetBuilderT::createSPOSet sposet creation failed");

    if (optimize == "rotation" || optimize == "yes") {
#ifdef QMC_COMPLEX
        app_error() << "Orbital optimization via rotation doesn't support "
                       "complex wavefunction yet.\n";
        abort();
#else
        app_warning() << "Specifying orbital rotation via optimize tag is "
                         "deprecated. Use the rotated_spo element instead"
                      << std::endl;
        ...
        if (!sposet_ref.isRotationSupported())
            myComm->barrier_and_abort("Orbital rotation not supported with '" +
                sposet_ref.getName() + "' of type '" +
                sposet_ref.getClassName() + "'.");
        auto rot_spo = std::make_unique<RotatedSPOsT<T>>(
            sposet_ref.getName(), std::move(sposet));
    ...
    if (!spo_object_name.empty() && sposet->getName() != spo_object_name)
        app_warning() << "SPOSet object name mismatched! input name: "
                      << spo_object_name
...
template <typename T>
std::unique_ptr<SPOSetT<T>>
SPOSetBuilderT<T>::createRotatedSPOSet(xmlNodePtr cur)
{
    std::string spo_object_name;
    std::string method;
    OhmmsAttributeSet attrib;
...
#ifdef QMC_COMPLEX
    myComm->barrier_and_abort("Orbital optimization via rotation doesn't "
                              "support complex wavefunctions yet.");
    return nullptr;
#else
    std::unique_ptr<SPOSetT<T>> sposet;
    processChildren(
    ...
    if (!sposet->isRotationSupported())
        myComm->barrier_and_abort("Orbital rotation not supported with '" +
            sposet->getName() + "' of type '" + sposet->getClassName() + "'.");

    sposet->storeParamsBeforeRotation();
    auto rot_spo =
        std::make_unique<RotatedSPOsT<T>>(spo_object_name, std::move(sposet));
