...
SoaLocalizedBasisSetT<COT, ORBT>::evaluateVGL(
    const ParticleSet& P, int iat, vgl_type& vgl)
{
    const auto& IonID(ions_.GroupID);
...
SoaLocalizedBasisSetT<COT, ORBT>::evaluateVGH(
    const ParticleSet& P, int iat, vgh_type& vgh)
{
    const auto& IonID(ions_.GroupID);
    const auto& d_table = P.getDistTableAB(myTableIndex);
    const auto& dist = (P.getActivePtcl() == iat) ? d_table.getTempDists() :
...
SoaLocalizedBasisSetT<COT, ORBT>::evaluateVGHGH(
    const ParticleSet& P, int iat, vghgh_type& vghgh)
{
...
SoaLocalizedBasisSetT<COT, ORBT>::evaluateV(
    const ParticleSet& P, int iat, ORBT* restrict vals)
{
    const auto& IonID(ions_.GroupID);
    const auto& coordR = P.activeR(iat);
    const auto& d_table = P.getDistTableAB(myTableIndex);
...
SoaLocalizedBasisSetT<COT, ORBT>::mw_evaluateValue(
    const RefVectorWithLeader<ParticleSet>& P_list, int iat, OffloadMWVArray& v)
{
    for (size_t iw = 0; iw < P_list.size(); iw++)
...
template <class COT, typename ORBT>
void
SoaLocalizedBasisSetT<COT, ORBT>::evaluateGradSourceV(const ParticleSet& P,
    int iat, const ParticleSet& ions, int jion, vgl_type& vgl)
{
...
template <class COT, typename ORBT>
void
SoaLocalizedBasisSetT<COT, ORBT>::evaluateGradSourceVGL(const ParticleSet& P,
    int iat, const ParticleSet& ions, int jion, vghgh_type& vghgh)
{
