 ...
 */
template<typename T>
SPOSetBuilderFactoryT<T>::SPOSetBuilderFactoryT(Communicate* comm, ParticleSet& els, const PSetMap& psets)
...
template<>
std::unique_ptr<SPOSetBuilderT<float>> SPOSetBuilderFactoryT<float>::createSPOSetBuilder(xmlNodePtr rootNode)
{
  ...
  std::unique_ptr<SPOSetBuilderT<float>> bb;

  if (type == "composite")
  {
    app_log() << "Composite SPO set with existing SPOSets." << std::endl;
    bb = std::make_unique<CompositeSPOSetBuilderT<float>>(myComm, *this);
  }
  else if (type == "jellium" || type == "heg" || type == "free")
  {
    app_log() << "Free-particle SPO set" << std::endl;
    bb = std::make_unique<FreeOrbitalBuilderT<float>>(targetPtcl, myComm, rootNode);
  }
  else if (type == "sho")
  {
    app_log() << "Harmonic Oscillator SPO set" << std::endl;
    bb = std::make_unique<SHOSetBuilderT<float>>(targetPtcl, myComm);
  }
#if OHMMS_DIM == 3
  ...
  else if (type == "molecularorbital" || type == "mo")
  {
    ...
    if (targetPtcl.isSpinor())
      ...
      bb = std::make_unique<LCAOrbitalBuilderT<float>>(targetPtcl, *ions, myComm, rootNode);
  }
#endif // OHMMS_DIM==3
  PRE.flush();
...
template<>
std::unique_ptr<SPOSetBuilderT<double>> SPOSetBuilderFactoryT<double>::createSPOSetBuilder(xmlNodePtr rootNode)
{
  ReportEngine PRE(ClassName, "createSPOSetBuilder");
  std::string sourceOpt("ion0");
  std::string type("");
  ...
  std::unique_ptr<SPOSetBuilderT<double>> bb;

  if (type == "composite")
  {
    app_log() << "Composite SPO set with existing SPOSets." << std::endl;
    bb = std::make_unique<CompositeSPOSetBuilderT<double>>(myComm, *this);
  }
  else if (type == "jellium" || type == "heg" || type == "free")
  {
    app_log() << "Free-particle SPO set" << std::endl;
    bb = std::make_unique<FreeOrbitalBuilderT<double>>(targetPtcl, myComm, rootNode);
  }
  else if (type == "sho")
  {
    app_log() << "Harmonic Oscillator SPO set" << std::endl;
    bb = std::make_unique<SHOSetBuilderT<double>>(targetPtcl, myComm);
  }
#if OHMMS_DIM == 3
  else if (type.find("spline") < type.size())
  {
  ...
  else if (type == "molecularorbital" || type == "mo")
  {
    ...
    if (targetPtcl.isSpinor())
      ...
      bb = std::make_unique<LCAOrbitalBuilderT<double>>(targetPtcl, *ions, myComm, rootNode);
  }
#endif // OHMMS_DIM==3
  PRE.flush();
...
template<>
std::unique_ptr<SPOSetBuilderT<std::complex<float>>> SPOSetBuilderFactoryT<std::complex<float>>::createSPOSetBuilder(
    xmlNodePtr rootNode)
{
  ReportEngine PRE(ClassName, "createSPOSetBuilder");
  std::string sourceOpt("ion0");
  ...
  std::unique_ptr<SPOSetBuilderT<std::complex<float>>> bb;

  if (type == "composite")
  {
    app_log() << "Composite SPO set with existing SPOSets." << std::endl;
    bb = std::make_unique<CompositeSPOSetBuilderT<std::complex<float>>>(myComm, *this);
  }
  else if (type == "jellium" || type == "heg" || type == "free")
  {
    app_log() << "Free-particle SPO set" << std::endl;
    bb = std::make_unique<FreeOrbitalBuilderT<std::complex<float>>>(targetPtcl, myComm, rootNode);
  }
  else if (type == "sho")
  {
    app_log() << "Harmonic Oscillator SPO set" << std::endl;
    bb = std::make_unique<SHOSetBuilderT<std::complex<float>>>(targetPtcl, myComm);
  }
#if OHMMS_DIM == 3
  else if (type.find("spline") < type.size())
  {
  ...
  else if (type == "molecularorbital" || type == "mo")
  {
    ...
    if (pit == ptclPool.end())
      ...
      ions = pit->second.get();
    if (targetPtcl.isSpinor())
      bb = std::make_unique<LCAOSpinorBuilderT<std::complex<float>>>(targetPtcl, *ions, myComm, rootNode);
    else
      bb = std::make_unique<LCAOrbitalBuilderT<std::complex<float>>>(targetPtcl, *ions, myComm, rootNode);
  }
#endif // OHMMS_DIM==3
  PRE.flush();
...
template<>
std::unique_ptr<SPOSetBuilderT<std::complex<double>>> SPOSetBuilderFactoryT<std::complex<double>>::createSPOSetBuilder(
    xmlNodePtr rootNode)
{
  ReportEngine PRE(ClassName, "createSPOSetBuilder");
  std::string sourceOpt("ion0");
  ...
  std::unique_ptr<SPOSetBuilderT<std::complex<double>>> bb;

  if (type == "composite")
  {
    app_log() << "Composite SPO set with existing SPOSets." << std::endl;
    bb = std::make_unique<CompositeSPOSetBuilderT<std::complex<double>>>(myComm, *this);
  }
  else if (type == "jellium" || type == "heg" || type == "free")
  {
    app_log() << "Free-particle SPO set" << std::endl;
    bb = std::make_unique<FreeOrbitalBuilderT<std::complex<double>>>(targetPtcl, myComm, rootNode);
  }
  else if (type == "sho")
  {
    app_log() << "Harmonic Oscillator SPO set" << std::endl;
    bb = std::make_unique<SHOSetBuilderT<std::complex<double>>>(targetPtcl, myComm);
  }
#if OHMMS_DIM == 3
  else if (type.find("spline") < type.size())
  {
  ...
  else if (type == "molecularorbital" || type == "mo")
  {
    ...
    if (pit == ptclPool.end())
      ...
      ions = pit->second.get();
    if (targetPtcl.isSpinor())
      bb = std::make_unique<LCAOSpinorBuilderT<std::complex<double>>>(targetPtcl, *ions, myComm, rootNode);
    else
      bb = std::make_unique<LCAOrbitalBuilderT<std::complex<double>>>(targetPtcl, *ions, myComm, rootNode);
  }
#endif // OHMMS_DIM==3
  PRE.flush();
