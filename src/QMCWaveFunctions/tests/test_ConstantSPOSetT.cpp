...
TEST_CASE("ConstantSPOSetT", "[wavefunction]")
{
    ...
    const int norb = 2;
    using WF =
        WaveFunctionTypes<QMCTraits::ValueType, QMCTraits::FullPrecValueType>;
    using Real = WF::Real;
    ...
    GradVector grow0{Grad({-2.22222, -1.11111, 0.33333}),
        Grad({8.795388, -0.816057, -0.9238793})};
    GradVector grow1{Grad({2.22222, 1.11111, -0.33333}),
        Grad({-8.795388, 0.816057, 0.9238793})};
    ...
    auto sposet =
        std::make_unique<ConstantSPOSetT<Value>>("constant_spo", nelec, norb);
    ...
    sposet->evaluate_notranspose(
        elec, first_index, last_index, phimat, gphimat, lphimat);
    ...
    sposet_vgl2->evaluate_notranspose(
        elec, first_index, last_index, phimat, gphimat, lphimat);
